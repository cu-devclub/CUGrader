...
const createSchemas = (t: ReturnType<typeof useTranslations>) => {
  ...
  const questionSchema = z.object({
    name: z
      .string()
      .min(1, t("assignment.form.validation.question.name.required")),
    description: z.string(),
    template: z.string(),
    maxScore: z.coerce
      .number()
      .min(0, t("assignment.form.validation.question.maxScore.min")),
    answer: z.string(),
    testCode: z.string(),
  ...
  const assignmentSchema = z.object({
    ...
    number: z.coerce
      .number()
      .min(1, t("assignment.form.validation.number.min")),
    publish: z
      .string()
      .min(1, t("assignment.form.validation.publish.required")),
    due: z.string().min(1, t("assignment.form.validation.due.required")),
    languageIds: z
      .array(z.number())
      .min(1, t("assignment.form.validation.languages.min")),
    examMode: z.boolean(),
    ...
    questions: z
      ...
      .min(1, t("assignment.form.validation.questions.min")),
  });
...
type AssignmentFormData = z.infer<
  ReturnType<typeof createSchemas>["assignmentSchema"]
>;
...
export function AssignmentForm({
  ...
  const [{ data: supportedLanguages }, { data: availableGroups }] =
    useSuspenseQueries({
      queries: [
        {
  ...
  const confirmDiscard = () => {
    router.push(`/instructor/class/${classId}/assignments`); // For now since cancel() is not working
    cancel();
  ...
  const [toRemoveExistingFileIds, setToRemoveExistingFileIds] = useState(
    [] as number[]
  );
  const filteredExistingFiles = existingFiles.filter(
    (it) => !toRemoveExistingFileIds.includes(it.id)
  );
  ...
  const isExamMode = form.watch("examMode");
  console.log(isExamMode);
      ...
      <nav className="sticky top-0 bg-background flex flex-col shadow">
        <div className="flex justify-between items-center flex-1 py-3 px-12">
          ...
          <div className="flex gap-2">
            ...
            <Button
              className={
                isExamMode ? "bg-[#0F1C48] hover:bg-blue-900" : "bg-primary"
              }
              onClick={handleSave}
              disabled={isPending}
            >
              ...
              {isPending
                ? prefill
                  ? t("assignment.form.buttons.saving")
                  : t("assignment.form.buttons.creating")
                : prefill
                ? t("assignment.form.buttons.save")
                : t("assignment.form.buttons.create")}
            </Button>
          </div>
        </div>
        <div
          className={`h-2 border-b ${
            isExamMode ? "bg-[#0F1C48]" : "bg-primary"
          }`}
        ></div>
      </nav>
      <Form {...form}>
        ...
        >
          ...
          <section className="border rounded-xl overflow-clip flex flex-col gap-8">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="p-8">
                    <FormLabel>
                      {t("assignment.form.fields.name.label")}
                    </FormLabel>
                    <FormControl>
                      <input
                        className="text-3xl outline-offset-8 placeholder:text-muted-foreground/50"
                        placeholder={t(
                          "assignment.form.fields.name.placeholder"
                        )}
                        {...field}
                      />
                    </FormControl>
              ...
              <div className="flex flex-wrap col-span-3 gap-6">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>
                        {t("assignment.form.fields.number.label")}
                      </FormLabel>
                      <FormControl>
                        <Input
                          className="w-36"
                ...
                <div className="flex gap-2 items-start">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>
                          {t("assignment.form.fields.publish.label")}
                        </FormLabel>
                        <FormControl>
                  ...
                  <div className="self-start mt-7">dots</div>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>
                          {t("assignment.form.fields.due.label")}
                        </FormLabel>
                        <FormControl>
                          <Input type="datetime-local" {...field} />
                        </FormControl>
                    ...
                    <FormItem className="flex gap-3">
                      <FormControl>
                        <Switch
                          ...
                          className={cn(
                            "transition",
                            "data-[state=checked]:bg-[#0F1C48]",
                            "data-[state=unchecked]:bg-gray-300"
                          )}
                        />
                      </FormControl>
                      <FormLabel>
                        {t("assignment.form.fields.examMode.label")}
                      </FormLabel>
                    </FormItem>
                  )}
                />
                ...
                {form.watch("examMode") && (
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>
                          {t("assignment.form.fields.examPin.label")}
                        </FormLabel>
                        <FormControl>
                          <Input
                            placeholder={t(
                              "assignment.form.fields.examPin.placeholder"
                            )}
                            maxLength={6}
                            pattern="[0-9]{6}"
                            {...field}
                        ...
                        <FormItem className="flex gap-3">
                          <FormControl>
                            <Switch
                              ...
                              onCheckedChange={field.onChange}
                              className={cn(
                                "transition-color", // smooth change
                                "data-[state=unchecked]:!bg-gray-300",
                                isExamMode ? "!bg-[#0F1C48]" : "bg-primary"
                              )}
                            />
                          ...
                          <FormLabel>
                            {t(
                              "assignment.form.fields.allowLateSubmission.label"
                            )}
                          </FormLabel>
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem className="flex gap-3">
                          ...
                          <FormLabel>
                            {t("assignment.form.fields.showScoreOnLock.label")}
                          </FormLabel>
                        </FormItem>
                      )}
                    />
              ...
              <div className="grid grid-cols-1 gap-6 items-start">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>
                        {t("assignment.form.fields.testCode.label")}
                      </FormLabel>
                      <FormControl>
                        <Textarea
                          placeholder={t(
                            "assignment.form.fields.testCode.placeholder"
                          )}
                          className="font-mono w-[76%] h-36 resize-y"
                          rows={6}
                      ...
                      <FormDescription>
                        {isMultipleLanguages
                          ? t(
                              "assignment.form.fields.testCode.disabledMultipleLanguages"
                            )
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>
                        {t("assignment.form.fields.secretTestCode.label")}
                      </FormLabel>
                      <FormControl>
                        <Textarea
                          placeholder={t(
                            "assignment.form.fields.secretTestCode.placeholder"
                          )}
                          className="font-mono w-[76%] h-36 resize-y"
                          rows={6}
                          disabled={isMultipleLanguages}
                      ...
                      <FormDescription>
                        {isMultipleLanguages
                          ? t(
                              "assignment.form.fields.secretTestCode.disabledMultipleLanguages"
                            )
                          : t(
                              "assignment.form.fields.secretTestCode.description"
                            )}
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
            ...
            <div className="px-12 pb-12 flex flex-col gap-4">
              <h2 className="text-lg font-medium">
                {t("assignment.form.sections.attachments")}
              </h2>

              <div className="grid grid-cols-2 gap-6">
                <div
                  className="rounded-lg p-4 border-dashed border-2 cursor-pointer flex items-center justify-center min-h-48"
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-4">
              ...
              <div className="relative my-8 h-10 w-full">
                <div
                  className={`absolute top-1/2 left-0 w-full h-[3px] ${
                    isExamMode ? "bg-[#0F1C48]" : "bg-primary"
                  }`}
                />
                <Button
                  onClick={() => addQuestion()}
                  type="button"
                  className={`h-7 w-7 absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-white rounded px-3 py-1 shadow ${
                    isExamMode ? "bg-[#0F1C48] hover:bg-blue-900" : "bg-primary"
                  }`}
                >
                  +
  ...
  return (
    <section className="border rounded-xl overflow-clip flex flex-col gap-8">
      ...
      <div className="px-12 flex flex-col gap-8">
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>
                {t("assignment.form.question.fields.template.label")}
              </FormLabel>
              <FormControl>
                <Textarea
                  placeholder={t(
                    "assignment.form.question.fields.template.placeholder"
                  )}
                  className="font-mono resize-y min-h-24"
                  rows={4}
                  {...field}
      ...
      <div className="px-12 grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>
                {t("assignment.form.question.fields.secretTestCode.label")}
              </FormLabel>
              <FormControl>
                <Textarea
                  placeholder={t(
                    "assignment.form.question.fields.secretTestCode.placeholder"
                  )}
                  className="font-mono resize-y min-h-24"
                  rows={4}
                  {...field}
      ...
      <div className="px-12 space-y-4">
        <div className="flex items-center justify-between">
          <h4 className="text-lg font-semibold text-foreground">
            {t("assignment.form.question.testCases.title")}
          </h4>
          <Button
            type="button"
            size="sm"
          ...
          >
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-muted-foreground">
                {t("assignment.form.question.testCases.testCase", {
                  number: testcaseIndex + 1,
                })}
              </span>
              <Button
                type="button"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      {t("assignment.form.question.testCases.input")}
                    </FormLabel>
                    <FormControl>
                      <Textarea
                        placeholder={t(
                          "assignment.form.question.testCases.inputPlaceholder"
                        )}
                        className="font-mono resize-y min-h-24"
                        rows={2}
                        {...field}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      {t("assignment.form.question.testCases.output")}
                    </FormLabel>
                    <FormControl>
                      <Textarea
                        placeholder={t(
                          "assignment.form.question.testCases.outputPlaceholder"
                        )}
                        className="font-mono resize-y min-h-24"
                        rows={2}
                        {...field}
      ...
      <div className="px-12 pb-12 space-y-4">
        <div className="flex items-center justify-between">
          <h4 className="text-lg font-semibold text-foreground">
            {t("assignment.form.question.secretTestCases.title")}
          </h4>
          <Button
            type="button"
            size="sm"
        ...
        {secretTestcaseFields.map((testcase, testcaseIndex) => (
          ...
          >
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-muted-foreground">
                {t("assignment.form.question.secretTestCases.testCase", {
                  number: testcaseIndex + 1,
                })}
              </span>
              <Button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      {t("assignment.form.question.secretTestCases.input")}
                    </FormLabel>
                    <FormControl>
                      <Textarea
                        placeholder={t(
                          "assignment.form.question.secretTestCases.inputPlaceholder"
                        )}
                        className="font-mono resize-y min-h-24"
                        rows={2}
                        {...field}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      {t("assignment.form.question.secretTestCases.output")}
                    </FormLabel>
                    <FormControl>
                      <Textarea
                        placeholder={t(
                          "assignment.form.question.secretTestCases.outputPlaceholder"
                        )}
                        className="font-mono resize-y min-h-24"
                        rows={2}
                        {...field}
