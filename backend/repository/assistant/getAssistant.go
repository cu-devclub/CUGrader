...
func GetAssistantsAndInstructors(classID int) ([]Info, []Info, error) {
	// Assistants
	assistantsQuery := `
		...
		WHERE ca.class_id = $1`
	rows, err := db.YSQL.Query(assistantsQuery, classID)
	if err != nil {
		return nil, nil, err
	}
	defer rows.Close()
	...
	instructorsQuery := `
		...
		WHERE c.id = $1`
	instructorRows, err := db.YSQL.Query(instructorsQuery, classID)
	if err != nil {
		return nil, nil, err
	}
