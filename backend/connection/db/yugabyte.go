...
func InitYugabyte(dsn string) {
	if dsn == "" {
		log.Println("YSQL_DSN is empty, skipping Yugabyte init")
		return
	}
	...
	for attempts := 1; attempts <= 20; attempts++ {
		...
		if err == nil {
			log.Println("Successfully connected to Yugabyte/Postgres DB")
			YSQL = database
			if _, err := database.Exec(schemaDDL); err != nil {
				log.Printf("Schema init notice: %v", err)
			} else {
				log.Println("Database schema verified/initialized successfully")
			}
			return
		}

		log.Printf("Yugabyte connection attempt %d/20 failed: %v. Retrying in 2s...", attempts, err)
		time.Sleep(2 * time.Second)
	}

	log.Fatalf("Yugabyte connection failed after retries: %v", err)
}
