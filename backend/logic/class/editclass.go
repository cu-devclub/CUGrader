...
func EditClass(id int, pictureFile *multipart.FileHeader, csvFile *multipart.FileHeader, updates map[string]interface{}) error {
	...
	if pictureFile != nil {
		...
		path, picture_id, err := class.GetPicturePathByClassID(id)
		if err != nil {
		...
		if picture_id == 0 {
			...
			if err := class.Edit(id, updates); err != nil {
	...
	err := class.Edit(id, updates)
	if err != nil {
		return fmt.Errorf("failed to edit class: %v", err)
	}

	if csvFile != nil {
		current, err := class.GetClassStudentMap(id)
		if err != nil {
		...
		for _, row := range records[1:] {
			...
			for csid, v := range current {
				...
				dbStudentID, err := class.GetStudentId(csid)
			...
			if foundCSID == 0 {
				...
				if err := class.InsertStudentIfNotExist(userID, studentID); err != nil {
					return err
				}
				sectionNum, _ := strconv.Atoi(sectionStr)
				sectionID, err := class.InsertSectionIfNotExist(id, sectionNum)
				if err != nil {
					return err
				}
				var groupID *int
				...
				if groupStrTrim != "" && groupStrTrim != "-" {
					gid, err := class.InsertGroupIfNotExist(id, groupStrTrim)
					if err != nil {
						return err
					}
					groupID = &gid
				}
				if err := class.InsertClassStudent(id, userID, sectionID, groupID); err != nil {
					return err
				}
			} else {
				...
				if foundSection != sectionStr {
					...
					if err := class.UpdateClassStudentSection(foundCSID, sectionID); err != nil {
						return err
					}
				}
				// Group update
				...
				if foundGroup != groupStrTrim {
					...
					if err := class.UpdateClassStudentGroup(foundCSID, groupID); err != nil {
						return err
					}
				}
			}
		...
		err = class.DeleteClassStudent(current, seenCSID)
		if err != nil {
			return err
		}
